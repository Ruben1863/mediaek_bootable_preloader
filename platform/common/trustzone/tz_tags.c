    ...
    u32 tz_sec_mem_end)
{
    ...
    UPDATE_RESERVE_MEM_TAG(SEC_MEM_TPLAY_MEMORY_SIZE,
        TZ_TAG_RESERVED_MEM_TPLAY_BUF);
    UPDATE_RESERVE_MEM_TAG(SEC_MEM_TPLAY_TABLE_SIZE,
        TZ_TAG_RESERVED_MEM_TPLAY_TABLE);
#if CFG_TEE_SECURE_MEM_SHARED
...
#if CFG_GZ_REMAP_OFFSET_ENABLE
#if (CFG_GZ_SUPPORT && CFG_GZ_REMAP)
    tag->u.sec_hw_cfg.remap_offset = GZ_REMAP_BASE_OFFSET;
#else
...
static void tz_secmem_tags_auto_verify(unsigned long param_addr)
{
    ...
    ASSERT_NONNULL(find_tag(param_addr, TZ_TAG_RESERVED_MEM_TPLAY_TABLE));
    ASSERT_NONNULL(find_tag(param_addr, TZ_TAG_RESERVED_MEM_TPLAY_BUF));
#if CFG_TEE_SECURE_MEM_SHARED
    ...
    unsigned int tz_sec_mem_start, unsigned int tz_sec_mem_end)
{
    ...
    tag = tz_update_sec_mem_cfg(tag, tz_sec_mem_start);
    tag = tz_update_reserved_mem(tag, tz_sec_mem_end);
    tag = tz_update_flash_dev_info(tag);
    tag = tz_update_sec_hardware_cfg(tag);
